...
for (l in 1:nrow(mapped_markers_and_inds) ){ # loop per marker
  ...
  # curr_marker_original_name <- "1_133127_3959"
  ...
  # print(see.group.data(group = g0, data.type = "PED"))
  ...
  # f1 <- cross.combinations(first.parents = curr_ind_id, second.parents = curr_ind_id, give.names = TRUE, name.prefix = "f1", save.genotype= TRUE, offspring = f1_num_offsprings)
...
#  curr_marker <- UpdateMarkerName(curr_marker)

  # print(f1_info$G)
  print(curr_marker_ind_in_map)
  # substr((f1_info$G), 84,85)
  # f1_info$G <- substr((f1_info$G), 84,85)
  ...
  if(fitt_t_test_res$p.value <= 0.05){
    ...
    # bc1 <- cross.combinations(first.parents = elite_line_id ,second.parents = as.integer(2)) #
    bc1 <- cross.combinations(first.parents = elite_line_id ,second.parents = new_f1_group) # 
    # bc1 <- cross.combinations(first.parents = curr_ind_id ,second.parents = new_f1_group) #
    
    # bc1_info <- data.frame(Index=see.group.data(bc1,data.type ="X"), GEBV=see.group.data(bc1,data.type = "BV"), G=see.group.data(bc1,data.type ="G"))
    # bc1_with_marker_individuals <- getIndsWithMarkerFromInfo(bc1_info, curr_marker_ind_in_map, "with_marker")
    # new_bc1_group <- make.group(bc1_with_marker_individuals[1])   
    # bc1S1 = self.n.times(new_bc1_group, n = 1, offspring = number_of_offsprings_bc1S1_S2_S3)
    #     bc1S2 = self.n.times(bc1S1,n = 1, offspring = number_of_offsprings_bc1S1_S2_S3)
    # bc1S3 = self.n.times(bc1S2,n = 1, offspring = number_of_offsprings_bc1S1_S2_S3)


    # curr_marker_ind_in_map_test <- 84
    ...
    # substr((bc1_group_info$G), curr_marker_ind_in_map*2-1, curr_marker_ind_in_map*2)
    # getIndsWithMarkerFromInfo(bc1_group_info, curr_marker_ind_in_map, "with_marker")
    ...
    print(see.group.data(bc1S4, "X")) ## Indexes (different from IDs !!)
    ...
    # bc1S4_t_test_res      <-  t.test(effect_sizes_with_marker,effect_sizes_without_marker,var.equal=TRUE) # t.test(x,y,var.equal=TRUE)
    ...
    if(bc1S4_fitt_t_test_res$p.value <= 0.05) {
      ...
      bc1s4_res <- c(curr_ind_id, max(inds_with_marker$GEBV)) ## what should be collected ? index of individual_with_marker | effect size of the marker?
...
# see.existing.groups()
# see.group.data(group = g0, data.type = "X")
# see.group.data(group = g0, data.type = "G")
# 
# see.group.data(group = f1, data.type = "X")
# see.group.data(group = new_f1_group, data.type = "X")
# see.group.data(group = new_f1_group, data.type = "G")
# see.group.data(group = new_f1_group, data.type = "N")

# make.group() ## Assign certain genotypes to a new group
# see.group.data(f1,"N" )
# see.group.data(g0,"N" )
# see.existing.groups()

# make.group(info[order(info$Pheno, decreasing¼TRUE),][1: n, Index])
# info <- data.table(Index¼see.group.data(group,”X”),
